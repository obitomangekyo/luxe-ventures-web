...
export function GuestCountField({
  ...
  const state = useNumberFieldState({
    ...
    locale: "en-US",
    onChange: (nextValue) => onChange(nextValue.toLocaleString("en-US")),
  });
  ...
  return (
    <InputGroup
      className={cn(
        "h-12 rounded-none border border-black/20 shadow-none focus-within:border-brand-detail focus-within:ring-0",
    ...
    >
      <InputGroupButton
        ...
        className="size-11 rounded-none text-3xl text-brand-text/60 hover:bg-brand-detail/20 hover:text-brand-text"
      >
        -
      </InputGroupButton>
      <InputGroupInput
      ...
      <InputGroupButton
        ...
        aria-disabled={!state.canIncrement}
