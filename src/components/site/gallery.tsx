...
export function Gallery() {
  return (
    <section id="gallery" className="bg-[#f3eee7]">
      <div className="container mx-auto px-5 py-28 xl:px-10 xl:py-40">
        <div className="flex flex-col justify-between gap-8 md:flex-row md:items-end">
          ...
          >
            Follow @{env.VITE_CONTACT_INSTAGRAM} <ArrowNE className="w-3.5" />
          </a>
        </div>
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-5">
          {gallery.map((image) => (
            <div className={cn("overflow-hidden", image.className)} key={image.alt}>
              <OptimizedImage
                asset={image.asset}
                alt={image.alt}
                loading="lazy"
